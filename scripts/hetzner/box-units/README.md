...
`sync-infra.sh` deliberately does **not** generate (it skips `loki-app`
...
| File | Box path | Owner |
|------|----------|-------|
| `loki-app.service` | `/etc/systemd/system/loki-app.service` | prod Next server, port 4002 |
| `Caddyfile.snippet` | blocks inside `/etc/caddy/Caddyfile` | loki + bridge vhosts |
...
```bash
# systemd unit
scp scripts/hetzner/box-units/loki-app.service root@167.233.22.31:/etc/systemd/system/
ssh root@167.233.22.31 'systemctl daemon-reload && systemctl restart loki-app'
