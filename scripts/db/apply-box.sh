#!/usr/bin/env bash
# apply-box.sh — apply a SQL/DDL file to the box Loki DB, THE RIGHT WAY.
#
# WHY: the reflex — `ssh box; sudo -u postgres psql -d loki -f x.sql` —
# creates objects OWNED BY postgres. `information_schema` is privilege-filtered,
# so the `loki` app role sees zero columns on those tables: the deploy
...
# object owned by `loki` automatically — no ALTER OWNER, no invisibility.
#
# It connects with the app's own DATABASE_URL (read from the box .env), so the
# CREATE runs as `loki` over TCP even though the ssh is root.
#
