...
# into the `loki` database, the `loki-app` systemd unit, and the
...
[ -n "${_LOKI_BRAND_SOURCED:-}" ] && return 0
_LOKI_BRAND_SOURCED=1

APP_NAME="Loki"                       # display string
APP_SLUG="loki"                       # lowercase kebab; URLs, paths, infra ids
APP_DOMAIN="loki.orangecat.ch"        # canonical hostname (no scheme)
...
# Resolve APP_<SUFFIX> → LOKI_<SUFFIX> → COCKPIT_<SUFFIX> (legacy).
# Usage: token="$(_brand_env SESSION_TOKEN)"
_brand_env() {
  local suffix="$1"
  local fallback="${2:-}"
  local app="APP_${suffix}"
  local fleet="LOKI_${suffix}"
