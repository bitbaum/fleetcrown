...
This is the native desktop application — the **local runtime** for FleetCrown
...
- **Ships in web-shell mode**: the main window loads
  `https://fleetcrown.orangecat.ch` directly and the user gets the exact same
...
- **Owns every agent PTY.** A dispatch claimed from `pending_commands` spawns
  ...
  is bundled, no `FLEETCROWN_RUNNER_PTY` switch exists.
...
- The embedded `home/watcher` reacts to `~/.fleetcrown/sessions/*.md` changes
...
FLEETCROWN_WEB_URL=https://fleetcrown.orangecat.ch ./Fleet-Runner-0.8.19.AppImage

# Or against a local dev server
FLEETCROWN_WEB_URL=http://localhost:3000 ./Fleet-Runner-0.8.19.AppImage
```

## Get the runnable app

End users should grab a signed installer from the
[download page](https://fleetcrown.orangecat.ch/download). For a local build
from source:
...
On launch, the app loads the FleetCrown control plane directly. Sign in with
...
- The headless box runner (`scripts/box-runner.ts`,
  `fleetcrown-box-runner.service` on Hetzner) reuses this runner core without
