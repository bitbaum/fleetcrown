...
export async function transcribeWithGroq(audio: File): Promise<TranscribeAttempt> {
  ...
  try {
    ...
    if (/groq transcribe 429/i.test(msg) || /rate.?limit/i.test(msg) || /quota/i.test(msg)) {
      return { ok: false, recoverable: true, status: 429, error: "Groq rate-limited or over quota.", detail: msg };
    }
    if (/abort|timeout/i.test(msg)) {
      return { ok: false, recoverable: true, status: 504, error: "Groq transcription timed out (>30s).", detail: msg };
    }
