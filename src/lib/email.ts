    ...
    <tr><td align="center">
      <table width="540" cellpadding="0" cellspacing="0" style="max-width:540px;width:100%;">
        ...
        <tr><td style="background:#09090b;padding:20px 32px;border-radius:12px 12px 0 0;text-align:center;">
          <span style="color:#ffffff;font-size:17px;font-weight:700;letter-spacing:-0.3px;">⚡ Cockpit</span>
        </td></tr>
        <!-- Body -->
      ...
      <table width="540" cellpadding="0" cellspacing="0" style="max-width:540px;width:100%;margin-top:20px;">
        <tr><td style="text-align:center;color:#71717a;font-size:12px;line-height:1.6;">
          Cockpit &mdash; your life operating system<br>
...
export function verifyEmailTemplate(verifyUrl: string, name: string) {
  const subject = "Verify your Cockpit email";
  const html = emailShell(`
    <h2 style="margin:0 0 8px 0;font-size:22px;font-weight:700;color:#09090b;">Verify your email</h2>
    ${p(`Hi ${name}, welcome to Cockpit. Click the button below to verify your email address and activate your account.`)}
    ...
    ${small("This link expires in 24 hours. If you didn't create a Cockpit account, you can safely ignore this email.")}
  `);
  const text = `Hi ${name}, verify your Cockpit email:\n\n${verifyUrl}\n\nThis link expires in 24 hours.`;
  return { subject, html, text };
}

export function welcomeEmailTemplate(name: string) {
  const url = appUrl();
  const subject = "Welcome to Cockpit";
  const html = emailShell(`
    <h2 style="margin:0 0 8px 0;font-size:22px;font-weight:700;color:#09090b;">You're in, ${name} ⚡</h2>
    ${p("Cockpit is your command center for everything that matters — projects, goals, habits, people, money, and AI agents.")}
...
export function resetPasswordEmailTemplate(resetUrl: string) {
  const subject = "Reset your Cockpit password";
  const html = emailShell(`
    <h2 style="margin:0 0 8px 0;font-size:22px;font-weight:700;color:#09090b;">Reset your password</h2>
    ${p("Someone requested a password reset for your Cockpit account. Click the button below to set a new password.")}
