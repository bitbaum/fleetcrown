 ...
 *
 * Controls the cockpit-daemon.service systemd user unit. Local-only —
 * Vercel cloud has no daemon to control, so the route 403s when
 * isRuntimeAvailable() is false. This is the one-click recovery path
 * the user surfaced 2026-05-31: until now, when the daemon went offline
 * the only recovery was a terminal command, which is product-fatal for
 * the "fleet control" value prop.
 *
 * Security: the route shells out to systemctl. We hard-code the unit
 * name + action whitelist; no user input is interpolated into the shell
 * command. RUNTIME_AVAILABLE is the local-machine gate (Vercel never
 * sets it, so cloud users can never reach the shell-out path).
 */
...
const UNIT = "cockpit-daemon.service";

async function systemctlState(): Promise<"active" | "inactive" | "failed" | "unknown"> {
  try {
    const { stdout } = await execp(`systemctl --user is-active ${UNIT}`, { timeout: 5000 });
    const s = stdout.trim();
...
export async function GET() {
  ...
  return NextResponse.json({ state, unit: UNIT });
}
...
export async function POST(req: NextRequest) {
  ...
  if (action === "status") {
    return NextResponse.json({ state: await systemctlState(), unit: UNIT });
  }
  ...
  try {
    ...
    const state = await systemctlState();
    return NextResponse.json({ ok: true, action, state, unit: UNIT });
  } catch (e: unknown) {
