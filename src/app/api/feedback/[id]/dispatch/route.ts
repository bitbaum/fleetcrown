...
const IMPLEMENT_ADAPTERS = ORCHESTRATION_ADAPTER_IDS.filter(
  (id) => id !== "openclaw",
);

function resolveImplementAdapter(
  agentPref: string | null | undefined,
): AdapterId {
  if (
    agentPref &&
    (IMPLEMENT_ADAPTERS as readonly string[]).includes(agentPref)
  ) {
