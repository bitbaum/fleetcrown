...
/**
 * Serves the @loki/agent CLI script body so a new customer can install
 * it from the cloud without needing the package to be published on npm.
 *
 * The package lives at packages/agent/bin/loki-agent.js and is NOT
...
export async function GET() {
  try {
    const filePath = path.join(process.cwd(), "packages", "agent", "bin", "loki-agent.js");
