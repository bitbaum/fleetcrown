  ...
  return (
    <PageLayout title={article.title} subtitle={article.summary} maxWidth="max-w-5xl">
      <article className="ui-panel-raised space-y-6 p-6 md:p-8">
        {article.body.split("\n\n").map((block, i) => {
          if (block.startsWith("## ")) return <h2 key={i} className="text-2xl font-medium text-text-primary">{block.replace(/^##\s+/, "")}</h2>;
          if (block.startsWith("- ")) return <ul key={i} className="list-disc pl-6 text-base md:text-lg text-text-secondary">{block.split("\n").map((line) => <li key={line}>{line.replace(/^-\s+/, "")}</li>)}</ul>;
          return <p key={i} className="text-base md:text-lg text-text-secondary leading-relaxed">{block}</p>;
        })}
      </article>
    </PageLayout>
  );
