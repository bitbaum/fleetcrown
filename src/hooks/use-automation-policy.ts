 ...
 * Refetches on LOKI_REFRESH_EVENT so per-project overrides and
...
export function useAutomationPolicy() {
  ...
  useEffect(() => {
    reload();
    window.addEventListener(LOKI_REFRESH_EVENT, reload);
    return () => window.removeEventListener(LOKI_REFRESH_EVENT, reload);
  }, [reload]);
