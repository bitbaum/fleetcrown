...
export function HabitRow({
  ...
  const startEdit = () => {
    setEditError(null);
    ie.start({ title: habit.title, frequency: habit.frequency });
  };
  ...
  const commitEdit = () => {
    ...
    setEditError(null);
    ie.commit(async () => {
      const ok = await onEdit(habit.id, {
  ...
  return (
    <>
      {ie.editing ? (
        ...
        <div className="group flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <button
              onClick={startEdit}
              className="ui-btn-row-action ui-hover-reveal"
              title="Edit habit"
      ...
      {editError && <p className="ui-error-xs pl-14">{editError}</p>}
    </>
  );
}
