 ...
 * bar's label; keep the real heading screen-reader-visible so mobile pages do
 * not lose their document heading. Detail/flow pages (e.g. "Start a new
 * project", an essay title) keep their visible h1 because the top bar shows
 * the parent label, not their title.
 */
export function PageTitle({ title }: { title: string }) {
  ...
  return <h1 className={cn("ui-page-title", duplicatesTopBar && "sr-only md:not-sr-only md:block")}>{title}</h1>;
}
