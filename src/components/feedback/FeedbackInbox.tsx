...
export function FeedbackInbox() {
  ...
  const { data, loading, error: loadError, refetch } = useFetch<{ feedback: InboxItem[]; metrics: FeedbackLoopMetrics | null }>(
    "/api/feedback/inbox",
  );
  ...
  if (all.length === 0 && loadError) {
    return (
      <EmptyState icon={AlertTriangle} title="Couldn't load feedback">
        ...
        <button type="button" onClick={refetch} className="text-accent-text underline-offset-2 hover:underline">
          Try again
        </button>
        .
      </EmptyState>
