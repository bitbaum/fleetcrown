...
export async function DigestsView({
  userId,
  window,
  project,
}: {
      ...
      <Card className="space-y-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {DIGEST_WINDOWS.map((digestWindow) => (
                <Link
                  key={digestWindow}
                  href={hrefFor(digestWindow, digest.projectKey)}
                  className={cn(
                    "ui-chip-filter",
                    digest.window === digestWindow && "ui-chip-filter-active",
                  )}
                >
                  {WINDOW_LABEL[digestWindow]}
                </Link>
              ))}
            </div>
          </div>
        ...
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            ...
            {digest.projects.map((digestProject) => (
              <Link
                key={digestProject.key}
                href={hrefFor(digest.window, digestProject.key)}
                className={cn("ui-chip-filter", digest.projectKey === digestProject.key && "ui-chip-filter-active")}
              >
                {digestProject.label}
              </Link>
            ))}
      ...
      {!hasActivity ? (
        ...
        <>
          ...
          <Card className="space-y-3">
            <div>
              <h2 className="text-sm font-semibold text-text-primary">Source Timeline</h2>
            ...
            <div className="space-y-1">
              {digest.timeline.map((item) => (
                <div
                  key={item.id}
                  className="grid gap-2 rounded-lg px-3 py-2 md:grid-cols-[8rem_9rem_minmax(0,1fr)]"
                >
