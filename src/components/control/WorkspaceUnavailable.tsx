    ...
    <div className="app-page app-page-compact app-viewport-pane flex flex-col gap-4">
      ...
      <div className="ui-callout-warning">
        ...
        <div className="space-y-3 text-sm leading-relaxed text-text-secondary">
          <p>{message}</p>
          <p>
            Use <Link href="/terminal" className="text-accent underline">Terminal</Link> → Cloud or This computer to watch and type into agent sessions, or dispatch from{" "}
            <Link href="/control" className="text-accent underline">Control</Link> / <Link href="/loki" className="text-accent underline">Loki</Link>.
          </p>
          {code === "cloud-builder-private" && (
            <p>
              Hosted sandboxes are private beta — connect{" "}
              <Link href="/download" className="text-accent underline">Fleet Runner</Link> on this computer for the default execution path.
            </p>
          )}
        </div>
      </div>
