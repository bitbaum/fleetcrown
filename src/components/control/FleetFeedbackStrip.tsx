...
export function FleetFeedbackStrip() {
  ...
  // undefined = no manual choice yet → auto-open the first project;
  // null = user explicitly closed; string = user's pick. Derived instead of
  // a setState-in-effect so the first render already shows the open panel.
  const [selection, setSelection] = useState<string | null | undefined>(undefined);
  const openProjectId = selection === undefined ? summary[0]?.projectId ?? null : selection;
  const setOpenProjectId = (next: (prev: string | null) => string | null) =>
    setSelection(next(openProjectId));
