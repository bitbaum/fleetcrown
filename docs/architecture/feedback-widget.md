...
| Widget modes (seam) | Embed panel is **Loki-on-the-site**: `data-fc-modes` enables Report (shipped) / Chat / Watch tabs (`widget/surface-modes.ts`). Chat + session Watch are progressive — Report still files Implement work today |
