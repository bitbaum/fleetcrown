-- Migration: user_projects.auto_ship — may Loki merge the pull request
...
-- TRUE = Loki merges that PR (and only that PR) when GitHub says it is
...
-- sites by construction — Loki cannot tell a client site from its own
