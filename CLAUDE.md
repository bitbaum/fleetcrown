...
| Feedback | /feedback | Reports your projects RECEIVED, in one ironing-out loop: what phase each fix is in and the next action, without opening a project. Fed by the embeddable widget (`widget/`, served as `/widget.js`), AI review, and synthesized briefs. `ControlInbox` is the preview; this is the surface |
| Feedback (sent) | /feedback/sent | The other side: reports YOU filed on anyone's site, with what came of each. Reporter's view, not operator's — same ladder as `/f/[token]`, no triage controls (a report you sent is not yours to implement). Rows bind to your account by possession of a track link or a verified email — never from the address on the submission itself; see `src/lib/feedback/claim.ts` |
| Tracked report | /f/[token] | PUBLIC. One report, for the person who filed it — no account, the track token is the credential (same story as `share/task/`). Often the first page of Loki anyone sees. What it may show is narrowed by `lib/feedback/public-status.ts`, never the operator's work phase: Shipped means the LIVE product changed, and a run error never crosses that wall. Read-only by construction — use `lib/feedback/public-view.ts`, never `attachFeedbackWork` (which calls GitHub and fires notifications) |
