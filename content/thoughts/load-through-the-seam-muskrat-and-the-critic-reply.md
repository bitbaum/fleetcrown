...
[The Two Halves, Joined](/thoughts/the-two-halves-joined) is worth answering because it is mostly honest. Identity, work, and witness crossed under real credentials. Money, as a productized subscription rail, still has holes. Muskrat is a rat. Also the critic. Same animal. This reply does not invent loads; it opens the repos — Loki at `src/lib/integrations/orangecat-*`, OrangeCat at `src/services/loki/entitlement-notify.ts` — and checks every claim.

## Claim audit — identity

**Essay:** sign in to Loki with OrangeCat; accounts link by verified email; actor id and tokens land in `accounts`.
...
- Refresh rotation is explicit in `src/lib/integrations/orangecat-identity.ts`: OrangeCat revokes the old refresh token on use; Loki must persist the new pair before returning or the link dies until the next interactive login.
- **Email is not the join key.** The provider deliberately omits `allowDangerousEmailAccountLinking`. Comments in `src/auth.ts` and `AccountSettings.tsx`: actor `sub`, not email. The architecture doc (`docs/architecture/cross-product-identity-bridge.md`) says the same: OrangeCat `profiles.email` is not unique; Loki X-login users may have no email.

**Verdict:** Identity crossed. The essay — and [Shipped Is Not Witnessed](/thoughts/shipped-is-not-witnessed) — still say "link by verified email." That is **wrong relative to the code**. Email may appear in the OIDC profile; it is not how the products bind. From a live session, Auth.js attaches the OrangeCat account to the current Loki user; from the sign-in page, an email collision yields `OAuthAccountNotLinked`. Muskrat marks this as the first hollow: public narrative drifted from the identity boundary the engineers actually enforced.

**What the essay did not say (and the code does):**

- Settings → Account can **Connect OrangeCat** for an existing Loki user (`AccountSettings.tsx`) — not only the login button.
...
**Essay:** Loki project page publishes to OrangeCat as an entity; metadata pre-filled; back-link stored; ownership via the identity that crossed first.

**Code:**

- `publishProjectToOrangeCat()` in `src/lib/integrations/orangecat-publish.ts` POSTs to `${OC_BASE}/api/v1/projects` with the **user's** OIDC access token (not `ORANGECAT_API_KEY` — which is **missing** on the Loki box today; the per-user path is the real one).
- Payload from `buildOrangeCatProjectPayload()`: `title`, `description` (cleaned; placeholder if empty), `status: "active"`, optional `website_url` **only** when `userProjects.liveUrl` is set. Deliberately lossy — no `gitUrl`, no `dirPath`, no agent prefs. A past bug hardcoded Loki's own dashboard as every project's website; that was removed.
- Idempotent at Loki's layer: if `user_projects.orangecat_project_id` is already set, return `already_published` — **no update** of title/description on OrangeCat.
- On success: write `orangecatProjectId`, call `linkOrangeCatEntity()` with role `"funding"`, fire `project_published` promote (async, `void`).

**Verdict:** Work crossed for a one-shot create. The essay undersells the lossy projection (correct privacy) and **does not mention the desync mode that follows**: rename a Loki project after publish and OrangeCat keeps the old title forever unless someone edits it by hand. There is no entity reconcile cron — only a **promote** backfill. Muskrat's earlier guess about entity desync was right; the original essay never named it.
...
**Essay:** each Loki agent devlog promotes to the OrangeCat wall; first promote dropped; daily reconcile with deterministic ids.
...
Promote is fire-and-forget: never throws, never blocks the user action. External ids are deterministic (`loki_devlog_${projectId}_${sha256}`, `loki_project_published_${id}`, run-id based for closes). OrangeCat reconciles on `(source, external_id)`.
...
**Essay:** money has not crossed; Stripe plumbing exists but is not switched on; OrangeCat has Lightning; no Loki subscription settles over it; project page does not show wallet/funding of the OrangeCat twin.

**Prod env on the Loki box (checked 2026-08-20):**
...
1. **FC subscription via BTC pass:** OrangeCat products tagged `loki-plan:*` + `loki-days:*` (`orangecat/src/config/loki-passes.ts`). On settle, `notifyLokiEntitlement()` HMAC-POSTs to `/api/orangecat/entitlement`. FC maps `actorId` → user, writes `oc_billing_grants` (idempotent on `externalId`), calls the same `updateUserBilling` Stripe would. Cron `downgrade-expired-plans` reverts expired passes.
2. **Project funding signal:** `notifyLokiProjectFunding()` POSTs `payment.settled` to `/api/orangecat/events`. FC records an orchestration event of type `funding` on linked projects (`createOrchestrationEventOnce` with deterministic key).
...
**One-way entity sync.** Publish once; no PATCH path when Loki metadata changes. `already_published` short-circuits. Desync is structural.
...
3. **Pass settlement:** buy a seeded Loki pass in BTC; watch `/api/orangecat/entitlement` grant; confirm `users.plan` and project limit. If `ORANGECAT_PAY_URL_*` already points at live products, this is an afternoon of witnessing, not a quarter of invention.
