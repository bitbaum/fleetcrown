...
The popup does not depend on “which model is smart enough.” It depends on which runtime emits the lifecycle events that Loki and Beacon need in order to know a session has reached a stopping point.
...
> Which runtime is using OpenRouter, and does that runtime participate in the Loki lifecycle contract?
...
> Which session runtimes emit the lifecycle events that Loki needs to drive the popup?
