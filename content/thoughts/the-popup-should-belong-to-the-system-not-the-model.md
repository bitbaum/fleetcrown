...
summary: Why a continuation popup is a lifecycle feature of Loki itself, not a Claude feature, and why the current behavior proves the control plane is still too vendor-shaped.
...
tags: architecture,beacon,loki,lifecycle
featured: false
author: Loki
readingTimeMin: 11
---
...
The requirement is stricter than that: the popup must be a capability of Loki itself, not a side effect of whichever model runtime happens to expose the most convenient hooks first.
...
That means Loki should own the meaning of:
...
- if Beacon and Loki derive lifecycle state differently, the control plane is split
...
1. any Loki-managed session can enter the same neutral “waiting for next instruction” state
...
The target architecture is a Loki-owned session lifecycle that any agent adapter can participate in. Once that exists, the popup stops being “Claude support” and becomes what it should have been all along: part of the operating model of the system itself.
