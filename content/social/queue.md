...
That improvised workflow is Loki's whole product thesis: not a better agent, but the missing dashboard for people already running agents next to their code. Who it's for, precisely:

https://loki.orangecat.ch/thoughts/the-levelsio-pattern-productized-who-loki-is-for

**LinkedIn**

There's a tweet from Pieter Levels that describes, step by step, the manual version of what Loki automates: Claude Code running on a VPS, SSH in to check on it, walk away while it works.

Loki is not a competitor to Cursor or Copilot. It's the productized version of a workflow indie hackers have been improvising for two years. The essay names the exact customer — the person with 3–15 projects, a laptop or a VPS, and no dashboard — and what they actually need from a control plane.
...
https://loki.orangecat.ch/thoughts/the-two-halves-joined

**LinkedIn**

In June we published a thesis: one person operating at superhuman scale needs two kinds of infrastructure — a production half for directing AI agent fleets, and a transaction half for settling the output without gatekeepers. We promised a follow-up when the first integration between Loki and OrangeCat actually shipped.
...
Claude Code, Codex, Grok Build — the single-agent tools are converging on a powerful worker substrate. Loki bets one tier up: on the captain.

But you cannot govern a fleet you do not run, and today Loki borrows the operator's laptop to run it. That borrowed ship is the honest keystone gap:

https://loki.orangecat.ch/thoughts/the-captain-needs-a-ship

**LinkedIn**

The AI coding tools are converging on a commodity: a very capable single worker. Loki's bet is one tier up — the captain's chair: governing many agents across many projects, with an economy attached, which no worker tool has.

The essay is mostly about the gap, not the moat: to be the captain, Loki must own the ship it currently borrows (the operator's own machine). Naming your keystone weakness in public is uncomfortable, which is exactly why the essay exists.
...
https://loki.orangecat.ch/thoughts/shipped-is-not-witnessed

**LinkedIn**

The Loki–OrangeCat bridge had been coded for weeks. Login, publish, promote — all reviewed, all deployed, all green. And not one real event had ever crossed it.
...
We're building the same flywheel sized for one person — Loki produces, OrangeCat settles:

https://loki.orangecat.ch/thoughts/the-techno-capital-machine-for-individuals
...
That loop has always required a corporation to host it. The essay asks what happens when you size the machine for an individual instead: an agent fleet as the production side (Loki), an AI-native economic layer as the settlement side (OrangeCat), and one operator running the whole loop on their own terms.
...
Once Loki and OrangeCat converge on one agent per user, one approval queue, one memory graph — why keep two products?

A systems-engineering answer to fuse, fork, or stay paired:

https://loki.orangecat.ch/thoughts/two-products-or-one
